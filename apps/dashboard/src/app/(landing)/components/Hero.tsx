...
export default function Hero() {
  const [viewMode, setViewMode] = React.useState<"preview" | "video">("video");
  ...
  return (
    <section className="relative pt-32 pb-24 overflow-hidden bg-gradient-to-b from-[#FFF1F2]/60 to-[#FAF9F7] select-none font-sans text-[#1C1917]">
      ...
      <div className="max-w-6xl mx-auto px-6 text-center space-y-8 flex flex-col items-center">
        ...
        {/* Browser Mockup Visual (HTML/CSS + YouTube Video Toggle) */}
        <div className="w-full max-w-4xl mt-12 bg-white rounded-xl border border-[#F1EDE9] shadow-lg overflow-hidden animate-fade-in relative z-15">
          {/* Browser Header Bar */}
          <div className="h-12 bg-[#FAF9F7] border-b border-[#F1EDE9] flex items-center justify-between px-4">
            ...
            {/* Toggle Tabs (Obvious Buttons) */}
            <div className="flex bg-[#F1EDE9]/60 p-1 rounded-xl border border-[#FAF9F7] text-xs space-x-1 shrink-0">
              <button
                onClick={() => setViewMode("video")}
                className={`px-4 py-2 rounded-lg font-bold transition-all duration-200 uppercase tracking-wider text-[9px] ${
                  viewMode === "video"
                    ? "bg-[#E11D48] text-white shadow-md scale-105"
                    : "text-[#78716C] hover:text-[#1C1917] bg-white/40 hover:bg-white/80"
                }`}
              >
                Watch Demo Video
              </button>
            ...
            <div className="hidden sm:block bg-white border border-[#F1EDE9] rounded px-6 py-0.5 text-[10px] text-[#78716C] font-mono select-none truncate max-w-xs font-semibold">
              {viewMode === "preview" ? "notifyflow.dev/dashboard/overview" : "youtube.com/watch?v=PiKvpfwErko"}
            </div>
          </div>
          ...
          <div className="transition-all duration-300">
            {viewMode === "preview" ? (
              ...
              <div className="aspect-video w-full bg-black">
                <iframe
                  src="https://www.youtube.com/embed/PiKvpfwErko?autoplay=1"
                  title="Notifyflow Demo Walkthrough"
