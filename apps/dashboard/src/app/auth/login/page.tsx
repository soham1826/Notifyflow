...
export default function LoginPage() {
  ...
  async function handleEmailLogin(e: React.FormEvent) {
    ...
    try {
      ...
      // 2. Call provision-tenant endpoint to ensure database registration
      ...
      if (!response.ok) {
        throw new Error(resData.error || "Failed to provision tenant credentials on backend.");
      }

      // 3. Store in localStorage for dashboard pages backward compatibility
      ...
      setError(err.message || "Something went wrong during login. Please check your credentials.");
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h2 className="mt-6 text-3xl font-extrabold tracking-tight text-[#1C1917]">
            Sign in to Notifyflow
          </h2>
          <p className="mt-2 text-sm text-[#78716C]">
            Manage your notification infrastructure and channels
          </p>
        </div>
        ...
        <div className="text-center text-sm text-[#78716C]">
          ...
          <Link href="/auth/signup" className="font-semibold text-[#E11D48] hover:underline underline-offset-4">
            Sign up free
          </Link>
        </div>
      </div>
